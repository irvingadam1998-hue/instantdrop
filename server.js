...
const rooms = new Map();
const deviceInbox = new Map();
const roomClips = new Map();
...
app.post('/api/send/:targetId', (req, res) => {
  ...
  upload.array('files')(req, res, (err) => {
    ...
    if (err && err.code === 'LIMIT_FILE_SIZE')
      return res.status(413).json({ error: 'Archivo muy grande (50MB máximo)' });
...
app.get('/api/inbox', (req, res) => {
  ...
  const items = [...getInbox(deviceId).values()]
    .map(({ id, name, size, mtime, fromEmoji }) => ({
      id, name, size, mtime, fromEmoji
    }));
...
app.delete('/api/inbox/:fileId', (req, res) => {
  ...
  if (!inbox.has(req.params.fileId))
...
app.get('/api/clips', (req, res) => {
  ...
  res.json([...clips.values()].map(({ id, text, mtime }) => ({
    id, text, mtime
  })));
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(' Solo dispositivos en la misma red WiFi podrán conectarse\n');
